...
$usernameGlobal="";
  /*
$serverName = "tritonserver.database.windows.net";
    $connectionOptions = array("Database" => "triton", 
        "Uid" => "tritonEffective", 
        "PWD" => "Howudoing1");
    $conn = sqlsrv_connect($serverName, $connectionOptions) or die( print_r( sqlsrv_errors(), true));
    try{
      $username="vvvvv5463";
      $tableName=strval($username);

    //$tsql= "CREATE TABLE '$tableName' ( followers VARCHAR(200), likedPosts VARCHAR(200), totalComments varchar(200)) ";

    $tsql='CREATE TABLE '.$tableName.' ' .
         '( followers VARCHAR(200), likedPosts VARCHAR(200), totalComments varchar(200))';
...
/*
  setcookie('va1','sdfdsf');
  
   setcookie('ca2','asdasdfsdfdsssssssssssssssssds');


  if(isset($_COOKIE['ca2'])){
      echo "<script>alert('try set')</script>";
  }

   if(isset($_COOKIE['va1'])){
      echo "<script>alert('try2 set')</script>";
  }
  */
  //print_r($_COOKIE)

/*
  $_SESSION['trySess']=100;
  $_SESSION['trySess2']=1002;
  
  if(isset($_SESSION['trySess'])){
    echo "<script>alert('trySess set')</script>";
  }
  if(isset($_SESSION['trySess2'])){
    echo "<script>alert('trySess2 set')</script>";
  }
  */
  function displaySignup(){
   ...
   // echo "<script>alert('$username'+' '+'$email'+' '+'$password'+' '+'$name'+' '+'$passwordRepeat');</script>";
    ...
    $var=uniqid("dbo.user");
    // echo $var;
    $tableName=strval($var);
    //echo ("<script>alert('$tableName');</script>");
  
    //$tableName=strval($email);
    

    //$tsql= "CREATE TABLE '$tableName' ( followers VARCHAR(200), likedPosts VARCHAR(200), totalComments varchar(200)) ";

    $tsql='CREATE TABLE '.$tableName.' ' .
         '( followers VARCHAR(200), following varchar(200) ,likedPosts VARCHAR(200), totalComments varchar(200), userPosts varchar(200), profilePhoto varchar(200), coverPhoto varchar(200), location varchar(200) )';
    ...
    if ($getResults == FALSE){
      ...
      $sql = "UPDATE dbo.allUsersInfo SET tableName='$tableName' WHERE username='$username' ";
    ...
    $tsql= "INSERT INTO $tableName (followers, following, likedPosts, totalComments, userPosts, profilePhoto, coverPhoto, location)
          VALUES ('-','-','-','-','-','css/images/profile_picture.png','css/images/cover.png','Blue Planet')";


    $getResults= sqlsrv_query($conn, $tsql);
    if ($getResults == FALSE){
      print_r(sqlsrv_errors());
       echo ("<script>alert('Some Error occured during default values enter');</script>");
       return "exit";
    }      
